 ...
 */
function PusherChatWidget(pusher, options, theUser, timezone, roomId, history) {
  ...
  this.settings = $.extend({
    ...
    chatEndPoint: '/pusher/chat.php', // the end point where chat messages should be sanitized and then triggered
  ...
  this._widget = PusherChatWidget._createHTML(this.settings.appendTo, this.user); // added user
  ...
  this._displayChatHistory(history);// persistence of chat messages
};
PusherChatWidget.instances = [];

/* @private */
PusherChatWidget.prototype._displayChatHistory = function(history) {
var self = this;
  if(history){
    [].forEach.call(history, function(el) {
      data = JSON.parse(el.activity);
      self._chatMessageReceived(data);
    });
  }
};
...
/**
* Find the current user's name and email, pass the data in 
* hidden fields so that the user doesn't need to type it in
*/

PusherChatWidget._createHTML = function(appendTo, theUser) {
  var html = '' +
  '<div class="pusher-chat-widget">' +
    '<div class="pusher-chat-widget-header">' +
      '<p><input type="hidden" name="nickname" value="'+ theUser.name +'" /></p>' +
      '<p><input type="hidden" name="email" value="'+ theUser.email +'" /></p>' +
    '</div>' +
    '<div class="pusher-chat-widget-messages">' +
      '<p><ul class="activity-stream">'+
      '</ul><p>' +
    '</div>' +
    '<div class="pusher-chat-widget-input">' +
      '<p><label for="message">Message</label></p>' +
      '<p><textarea name="message"></textarea></p>' +
      '<p><button class="pusher-chat-widget-send-btn">Send</button></p>' +
    '</div>' +
  '</div>';
  var widget = $(html);
